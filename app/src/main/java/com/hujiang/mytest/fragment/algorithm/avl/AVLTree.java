package com.hujiang.mytest.fragment.algorithm.avl;
...
public class AVLTree<K extends Comparable<K>, V> {
    ...
    private Node add(Node node, K key, V value) {
        ...
        //LL balanceFacotor >1，说明左边节点不平衡；getBalanceFactor(node.left),证明？
        if (balanceFacotor > 1 && getBalanceFactor(node.left) >= 0)
            //右旋转
            return rightRotate(node);
        //RR balanceFacotor < -1，说明右边节点不平衡；
        if (balanceFacotor < -1 && getBalanceFactor((node.right)) <= 0)
            //左旋转
            return leftRotate(node);
        //LR  balanceFacotor >1，说明左边节点不平衡；
        if (balanceFacotor > 1 && getBalanceFactor(node.left) < 0) {
            node.left = leftRotate(node.left);
            return rightRotate(node);
        }
        //RL balanceFacotor < -1，说明右边节点不平衡；
        if (balanceFacotor < -1 && getBalanceFactor(node.right) > 0) {
