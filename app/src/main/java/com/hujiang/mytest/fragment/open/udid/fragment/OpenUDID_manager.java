 ...
 * @author yuefeng
 * @version 3.4.1
 *          1、这里不仅仅提供一个generateOpenUDID方法生成一个udid， 还可以通过intent获取第三方库提供的uuid。
 *          2、这里也提供一个访问第三方service的方法，方便
 * @created 2017/2/15
 */
...
public class OpenUDID_manager implements ServiceConnection {
     ...
     * 启动service，获取远端service提供的udid。尝试每一个可用的Service ,遍历整个mMatchingIntents列表
     * 如果不通从远端service获取，就本地生成一个
     */
